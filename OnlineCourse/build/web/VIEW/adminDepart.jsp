...
<html>
    ...
    <body>
        ...
        <div class="page depart" style="text-align: center">  
            <div>
                <a href="/Project_PRJ301/ADServlet">user</a>
            </div>   
            <div>
                <a href="/Project_PRJ301/ADclassServlet">class</a>
            </div>
            <div style="background-color: black;">
                <a href="/Project_PRJ301/ADdepartServlet" style="color: white">department</a>
            </div>
        </div>
        ...
        <table class="table table-striped table-hover" style="margin-top: 10px;height: 65%;">
            <tr>
                <th colspan="6" style="text-align: center; background-color: black ; color: white" >Danh sách ngành học</th>
            </tr>
            <tr>
                <th>ID</th>
                <th>Name</th>
            ...
            <c:forEach items="${listDE}" var="st">
                <tr>
                    <td>${st.ID}</td>
                    <td>${st.name}</td>
                    <td>${st.title}</td>
                    <td>${st.images}</td>
                    <td><a href="UpdateDeServlet?id=${st.ID}">UPDATE</a></td>
                    <td><a href="ADdepartServlet?type=2&id=${st.ID}">DELETE</a></td>
                </tr>
            </c:forEach>
        </table>
        ...
        <div class="page" style="text-align: center">
            <c:forEach begin="1" end="${endp}" var="i" >
                <div>
                    <a href="ADdepartServlet?index=${i}">${i}</a>
                </div>
            </c:forEach>
        </div>
        <a class="insert" href="/Project_PRJ301/insertDeServlet">Thêm ngành</a>
        <div id="footer">
